    ...
    vim.ui.input({ prompt = 'Enter replacement text: ' }, function(replace_text)
      ...
      vim.ui.input({ prompt = "Enter line ranges (e.g., 10,20 30,40) or leave empty for whole file: " },
        function(range_input)
          ...
          vim.ui.select({ 'Confirm each', 'Replace all' }, { prompt = 'Choose replace mode:' }, function(choice)
            if choice == nil then
              print('Operation cancelled.')
              return
            end
            ...
            for _, range in ipairs(ranges) do
              local cmd = string.format("%ss/%s/%s/%s", range, esc(search_text), esc(replace_text), flag)
              vim.cmd(cmd)
            end
          end)
        end)
